...
const transformToLis = ((obj) => Object.keys(obj).map((value) => `<li>${value}: ${obj[value]}</li>`));
...
const count = (target, input) => (input.flat(Infinity)).filter((a) => a === target).length;
// Solution code here...
...
const totalSum = (input) => {
  // Solution code here...
  return input.flat().reduce((preval, currentVal) => preval + currentVal);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    arrayResults.push(innerArray.filter(num => ((num % 5 === 0) && typeof (num) === 'number')).map(num => Math.pow(2, num)));
  }
  return arrayResults;
  ...
  //     array.map((value) => {
  //       if (value % 5 === 0) {
  //         arrayResults.push(Math.pow(2,value));
  //       }
  //     });
  //   });
...
// Solution code here...
// data.filter(data => data.gender.includes('male');
