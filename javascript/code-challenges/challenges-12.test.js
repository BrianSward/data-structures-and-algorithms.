...
const hasNumber = (string) => {
  let reggie = /[a-zA-Z]+[0-9]+/;
  return reggie.test(string);
  // Solution code here...
};
...
const validatePhoneNumber = (phoneNumber) => /^\s*([(]\d{3}[) ]{2}(\d{3})[- ](\d{4})|(\d{3})[- ](\d{3})[- ](\d{4}$)|(\d{3})[- ](\d{7})|(\d{6})[- ](\d{4})|(\d{10}$))/.test(phoneNumber) ? true : false;
// Solution code here...
