...
const sortByChildren = (charArray) => {
  // should out to dan awesome for his help at our table here for the insight on using else ifs instead of trying ternarys the whole time
  function sorter(a, b) {
    let x = a;
    let y = b;
    let value = 0;
    ...
    if (x.children.length < y.children.length) {
      ...
      tempArr.push(y.name);
      tempArr.sort();
      (tempArr[0] === x) ? value = 1 : value = -1;
    }
    return value;
  }
  return charArray.sort(sorter);
};
...
const containsWorld = (input) => /\bworld\b/.test(input);
// Solution code here...
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  let temp = [];
  arr.forEach((v) => (/^[A-J]/.test(v) === true) ? temp.push(v) : false);
  return temp;
};
