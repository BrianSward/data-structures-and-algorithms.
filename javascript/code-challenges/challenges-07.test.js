...
const listFoods = (recipe) => {
  ...
  for (let i=0; i<recipe.ingredients.length; i++){
    let step1 = recipe.ingredients[i].indexOf(' ');
    let step2 = recipe.ingredients[i].slice(step1 + 1);
    let step3 = step2.indexOf(' ');
    let step4 = step2.slice(step3 + 1);
    result.push(step4);
  }
  return result;
};
